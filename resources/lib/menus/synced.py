...
class SyncedMenu:
        ...
        try:
            ...
            for index, diretory in enumerate(synced_dirs):
                ...
                if diretory["type"] == "single-movie":
                    ...
                    all_items += self.get_movies_in_directory(diretory["file"])
    ...
    def view(self):
        ...
        synced_dirs = self.database.get_synced_dirs()
        if not synced_dirs:
            xbmcgui.Dialog().ok(ADDON_NAME, STR_NO_SYNCED_DIRS)
            return
        lines = [
            f"[B]{x['label']}[/B] - {x.localize_type()} - [I]{x['file']}[/I]"
        ...
        ret = xbmcgui.Dialog().select(f"{ADDON_NAME} - {STR_SYNCED_DIRECTORIES}", lines)
        if ret >= 0:
